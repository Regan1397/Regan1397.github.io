(function (c) {
    ...
    var a = {
        init: function (f) {
            ...
            return this.each(function () {
                ...
                if (m.length <= g.scalethreshold && g.staticbelowthreshold) {
                    m.each(function (n) {
                        c(this).unbind("click.coverstand");
                        c(this).bind("click.coverstand", function () {
        ...
        prev: function () {
            return this.each(function () {
                var d = c(this);
                var prev = d.find("." + b.selectedclass + ":eq(0)");

                if (prev.attr('rel') == 1)
                    return false;

                 prev = prev.prev().trigger("click");
            })
        }
    };
    c.fn.coverstand = function (d) {
        if (a[d]) {
